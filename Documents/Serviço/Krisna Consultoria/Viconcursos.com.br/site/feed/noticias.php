...
$canal->addChild('title', 'Viconcursos  Noticias');
$canal->addChild('link', 'http://www.viconcursos.com');
$canal->addChild('description', 'Concursos Publicos e Apostilas - Quer Passar em Concursos?, Editais, Provas, Gabaritos, Resultados, Empregos, Estagios, dicas para concursos, videos para concursos, Dicas, Ofertas de Emprego.');
$canal->addChild('language', 'pt-br');


$dados = new noticias(array('idnoticias' => '', 'data' => '', 'titulo' => '', 'subtitulo' => ''));
$dados->setExtrasSelect('order by data desc');
$dados->selecionaCampos($dados);
while ($row = $dados->retornaDados()) {
    $id = $row->idnoticias;
    $data = date(DATE_RFC822, strtotime($row->data));
    $titulo = $row->titulo;
...
// Adiciona sub-elementos ao elemento <item>
    ...
    $item->addChild('link', BASEURL . "noticias/detalhes/" . $id . "/" . $titulo_tratado);
